...
# Multi-omics and Multi-blocks {#multi-omics}

This section will be explained with a multi-omics dataset from an exposure
...
```{r multiomics, echo = TRUE, eval = TRUE}
data(mouse_ds)
...
```{r multiomics-preprocess, echo = TRUE, eval = TRUE}
...
```{r multiomics-analysis, echo = TRUE, eval = TRUE}
resultsPCA <- ComDim_PCA(allMB, ndim = 2) # 2 Components.
```
...
ggplot(data = scoresTable_wider) +
  geom_point(aes(x = T.scores1, y = T.scores2, color = sample.type,
                 shape = block.name)) +
  geom_point(aes(x = Q.scores1, y = Q.scores2, color = sample.type,
...
ggsave(filename = "fig4_2b.png", height= 9.6, width=10.7, units='cm', bg= "white")
```
```{r plot2, echo = FALSE, eval = TRUE}
knitr::include_graphics("Figs/fig4_2b.png")
```

### Loadings

The **loadings** give the contribution of each variable in the model.
An example of how to use the loadings is presented in [**CHAPTER6**](https://f-puig.github.io/R.ComDim.Docs/Section5.html).
...
```{r multiomics-predict, echo = TRUE, eval = TRUE}
pred.resultsPCA <- PredictMultiBlock(MB = allMB, model = resultsPCA)
head(data.frame(original = resultsPCA@T.scores[[2]][,1],
           predicted = pred.resultsPCA@T.scores[[2]][,1]))
head(data.frame(original = resultsPCA@Q.scores[,2],
